...
const init = async () => {
    ...
    var minimumVaultSize = 1000000000000000000  //100000000000000000000   =   $1
    ...
    const exists = await contract.exists(vaultID);
        ...
        if (exists == true) 
            {const checkLiquidation = await contract.checkLiquidation(vaultID);
                console.log(vaultID)
                if (checkLiquidation == true)
                    {const checkCost = await contract.checkCost(vaultID);
                    console.log(vaultID+ "   $"+checkCost/1000000000000000000);
                    ...
                    {(async() => {
                        ...
                        const tx = {
                            ...
                            const transaction = await contract.liquidateVault(vaultID, tx);
