      ...
      <div className="mt-4 p-4 border border-gray-200 rounded-lg">
        <div>
          <div className="grid grid-cols-8 gap-2">
            ...
            {_.values(recurringPayments).map(([accountNumber, sender, recipientAddress, amount, token, timeIntervalSeconds, paymentInterface, paymentDue, cancelled, subscription]) => {
              ...
              return (
                <div key={accountNumber.toString()} className="grid grid-cols-8 gap-2">
                  ...
                  <div>
                    {!cancelled && (<Button size="sm" onClick={() => cancelRecurringPayment(accountNumber)}>Cancel</Button>)}
                  </div>
                </div>
              )
            })}
